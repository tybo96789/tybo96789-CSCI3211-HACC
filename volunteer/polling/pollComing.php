<?php
  ...
  $t_select = "SELECT TeamID FROM volunteer WHERE VolunID = '$volunID' LIMIT 1";
  $volunteer = $call::run($t_select);
  $teamID = "";
  foreach($volunteer as $v){$teamID = $v['TeamID'];}

  $r_select = "SELECT r.EventID, e.EventCode FROM registration r JOIN event e ON r.EventID = e.EventID  WHERE VolunID = '$volunID'";
  $e_reg = $call::run($r_select);
  $registered = "0";
  foreach($e_reg as $er){
    $registered .= ", " . $er['EventCode'];
  }

  $select = "SELECT * FROM event WHERE EventType ='PL' AND TeamID = $teamID AND EventCode NOT IN ($registered) ORDER BY EventCode;";
...
<html lang="en">
          ...
          <div class="container" id="wrap">
            ...
            <tbody>
              ...
              foreach($events as $e){
                ...
                echo"<td><a href =https://maps.google.com/?q=".urlencode($e['Location'])." target = '_blank'>".$e['Location']."</a></td>";
                ...
                // echo"<td>".$e['TeamID']."</td>";
                echo"<td><button id='". $e['EventID'] ."' class='btn btn-info' role='button'>Register</button></td>";
  ...
  <script>
    $("#event-listing button").on("click", function(){
      ...
      var et = $(this).parents('tr').children('td').eq(1).html();
